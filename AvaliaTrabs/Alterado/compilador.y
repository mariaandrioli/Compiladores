...
char const_declaracao_atual[20];
...
parte_declara: parte_declara_consts parte_declara_vars 
;

parte_declara_consts:  const 
;

const: CONST declara_consts
            |
;

declara_consts: declara_consts declara_const 
            | declara_const 
;

declara_const: id IGUAL 
              expressao 
            PONTO_E_VIRGULA {/* ARMZ */
               char exp[10];
               elemento_t elemento = malloc(sizeof(elemento_t));
               elemento = buscaTabela(tabelaSimbolos, const_declaracao_atual);
...
comandos: atrib_chamada PONTO_E_VIRGULA comandos |
         atrib_chamada PONTO_E_VIRGULA |
         atrib_chamada |
...
atrib_chamada: atribuicao | chamada
;
...
chamada: ABRE_PARENTESES com_params FECHA_PARENTESES | fator ;
...
fator: IDENT {
      ...
      if (elemento->categoria == constante){
         sprintf(exp, "CRVL %d, %d",  elemento->nivel_lex ,  elemento->endereco);
      }
